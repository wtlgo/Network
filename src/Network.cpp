...
void Network::set_proxy(const std::string& proxy) { this->proxy = proxy; }

std::string Network::request(std::string url,
    ...
    std::string fields;
    {
        ...
        for (const std::pair<std::string, std::string>& row : args) {
            tmp_list.push_back(url_encode(row.first) + "=" +
                               url_encode(row.second));
        }
...
bool Network::download(const std::string& url,
...
std::string Network::upload(std::string url, const std::string& fieldname,
    ...
    if (curl) {
        std::unique_ptr<curl_httppost, void (*)(curl_httppost*)> formpost(
            [&fieldname, &filename] {
                curl_httppost* formpost = nullptr;
                curl_httppost* lastptr = nullptr;

                curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME,
                             fieldname.c_str(), CURLFORM_FILE, filename.c_str(),
                             CURLFORM_END);
...
size_t Network::string_writer(char* contents, size_t size, size_t nmemb,
                              std::string* stream) {
    stream->append(contents, size * nmemb);
    return size * nmemb;
}

size_t Network::file_writer(void* contents, size_t size, size_t nmemb,
                            FILE* stream) {
