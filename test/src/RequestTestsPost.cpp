...
TEST(Request, Post_NoSSL_Static)
{
    ...
    string response = network.request("http://" + test_server + "/post", {{"hello", "hello"}}, true);
    ASSERT_NO_THROW({
...
TEST(Request, Post_SSL_Static)
{
    ...
    string response = network.request("https://" + test_server + "/post", {{"hello", "hello"}}, true);
    ASSERT_NO_THROW({
        auto resp = json::parse(response);
        ASSERT_EQ(resp["form"]["hello"], "hello");
    });
...
TEST(Request, Post_NoSSL_Random)
{
    ...
    ASSERT_NO_THROW({
        ...
        string response = network.request("http://" + test_server + "/post", {{arg, val}}, true);
        auto resp = json::parse(response);
...
TEST(Request, Post_SSL_Random)
{
    ...
    ASSERT_NO_THROW({
        ...
        string response = network.request("https://" + test_server + "/post", {{arg, val}}, true);
        auto resp = json::parse(response);
...
TEST(Request, Post_NoSSL_Random_MultipleArgs)
{
    ...
    ASSERT_NO_THROW({
        string response = network.request("http://" + test_server + "/post", args, true);
        auto resp = json::parse(response);
...
TEST(Request, Post_SSL_Random_MultipleArgs)
{
    ...
    string response = network.request("https://" + test_server + "/post", args, true);
    ASSERT_NO_THROW({
        auto resp = json::parse(response);
